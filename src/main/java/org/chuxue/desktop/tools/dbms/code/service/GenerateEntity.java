...
public class GenerateEntity {
	...
	public static void generate(SysDbmsGenerateCodeInfo sysDbmsGenerateCodeInfo, SysDbmsTabsInfo tabsInfo,
		...
		stringBuilder.append("@Table(name = \""
				+ tabsInfo.getTabsName().substring(tabsInfo.getTabsName().lastIndexOf(".") + 1) + "\")\r\n");
		stringBuilder.append("@NamedQuery(name = \"" + sysDbmsGenerateCodeInfo.getClassName()
				+ ".findAll\", query = \"SELECT s FROM " + sysDbmsGenerateCodeInfo.getClassName() + " s\")\r\n");
	...
	private static void spellMethod(StringBuilder stringBuilderMethod, String propertiesName, String propertiesType,
		...
		stringBuilderMethod.append("	 */\r\n");
		stringBuilderMethod.append(
				"	public void set" + upPropertiesName + "(" + propertiesType + " " + propertiesName + ") {\r\n");
	...
	private static void spellProperties(StringBuilder stringBuilderProperties, String propertiesName,
		...
		if ("date".equals(colsType.toLowerCase())) {
			...
			stringBuilderProperties
					.append("	@JsonFormat(locale = \"zh\", timezone = \"GMT+8\", pattern = \"yyyy-MM-dd\")\r\n");
			...
			stringBuilderProperties
					.append("	@JsonFormat(locale = \"zh\", timezone = \"GMT+8\", pattern = \"HH:mm:ss\")\r\n");
		...
		stringBuilderProperties
				.append("	@Column(name = \"" + colsName + ", columnDefinition = \" COMMENT '" + colsDesc + "\")\r\n");
