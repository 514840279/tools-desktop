...
import org.chuxue.desktop.tools.dbms.tabs.vo.SysDbmsTabsColsInfoVo;
...
public class ZhcxService {
	...
	private void resultMap(String sqlString, Map<String, String> param, SysDbmsTabsColsInfoVo vo,
		...
		if ("Oracle".equals(database)) {
			...
			pageSql.append(" )                           	   ");
			pageSql.append(
					" where tp_rownum > " + (vo.getPageNumber().intValue() - 1) * vo.getPageSize().intValue() + "  ");
		} else if ("MYSQL".equals(database)) {
			pageSql.append(
					sqlString.toString() + " limit " + (vo.getPageNumber().intValue() - 1) * vo.getPageSize().intValue()
							+ "," + vo.getPageSize().intValue());
		}

		System.out.println(pageSql.toString());
		// List<Map<String, Object>> list =
		// jdbcTemplate.queryForList(pageSql.toString());
		...
		if ("单表多条件查询".equals(vo.getType()) || "单表多条件更多查询".equals(vo.getType())) {
			...
			if ((vo.getTotal() == null || "0".equals(vo.getTotal().toString()))) {
				...
				// jdbcTemplate.queryForMap(countsql);
	...
	public Map<String, Object> findBySingleTableByGroupsAndMulteityParam(SysDbmsTabsColsInfoVo vo) {
		...
		for (SysDbmsTabsColsInfo sysDbmsTabsColsInfo : vo.getList()) {
			if (sysDbmsTabsColsInfo.getColsSort() != null
					&& ("asc".equals(sysDbmsTabsColsInfo.getColsSort().toLowerCase())
							|| "desc".equals(sysDbmsTabsColsInfo.getColsSort().toLowerCase()))) {
				if (!sortColumnFlag) {
					sql.append(
							" order by " + sysDbmsTabsColsInfo.getColsName() + " " + sysDbmsTabsColsInfo.getColsSort());
					sortColumnFlag = true;
				} else {
