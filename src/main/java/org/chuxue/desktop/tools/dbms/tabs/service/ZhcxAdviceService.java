...
public class ZhcxAdviceService {
	...
	public static void startConfixOracleTableCloumnIndexConfig(SysDbmsTabsInfo sysZhcxTab,
			...
			JdbcTemplate jdbcTemplate2, SysDbmsTabsColsInfo sysZhcxCol) {
		...
		NamedParameterJdbcTemplate template = new NamedParameterJdbcTemplate(
				multiDatasource.get(sysZhcxTab.getJdbcUuid()));
		SysDbmsAdviMessInfo advice = null;
		StringBuffer sBuffer = new StringBuffer();
		sBuffer.append(
				" select i.owner,i.index_name,i.table_owner,i.table_name,ic.column_name,i.tablespace_name from all_indexes i ");
		sBuffer.append(
		...
		if (resultlist != null && resultlist.size() > 0) {
			...
			if (!expactUser.equals(resultmap.get("owner"))
					|| !expactIndexSpaces.equals(resultmap.get("tablespace_name"))) {
				advice = new SysDbmsAdviMessInfo(UUID.randomUUID().toString(), "索引重建", sysZhcxTab.getTabsDesc(),
						sysZhcxTab.getTabsName(), sysZhcxTab.getJdbcUuid());
				sBuilder.append("-- 由于索引信息并不是期望的值，建议充建索引信息：\n");
				String indexName = "IND_" + UUID.randomUUID().toString().replace("-", "").substring(4, 20) + "_"
						+ sysZhcxCol.getColsOrder();
				sBuilder.append("-- 预期值：\t 索引所属：" + expactUser + "\t索引的表空间：" + expactIndexSpaces + "\n");
				sBuilder.append("-- 实际值：\t 索引所属：" + resultmap.get("owner") + "\t索引的表空间："
						+ resultmap.get("tablespace_name") + "\n");
				sBuilder.append("drop index " + resultmap.get("owner") + "." + resultmap.get("index_name") + ";\n");
				sBuilder.append(" create index " + expactUser + "." + indexName + " on " + tableName + " ("
						+ sysZhcxCol.getColsName() + ")  tablespace " + expactIndexSpaces + ";");

			} else {
				return;
			}
		} else if (sysZhcxCol.getUserIndex() != null && !"".equals(sysZhcxCol.getUserIndex())) {
			advice = new SysDbmsAdviMessInfo(UUID.randomUUID().toString(), "索引添加", sysZhcxTab.getTabsDesc(),
					sysZhcxTab.getTabsName(), sysZhcxTab.getJdbcUuid());
	...
	public static void startConfixOracleTableColumnsConfig(SysDbmsTabsInfo sysZhcxTab,
			Map<String, DataSource> multiDatasource, SysDbmsAdviMessInfoDao sysAdviceMessDao,
			JdbcTemplate jdbcTemplate2, List<SysDbmsTabsColsInfo> list) {
		...
		for (SysDbmsTabsColsInfo sysZhcxCol : list) {
			StringBuffer sBuffer = new StringBuffer();
			sBuffer.append(
					" select tc.OWNER,tc.TABLE_NAME,tc.COLUMN_NAME,cc.comments,tc.NUM_NULLS,tc.DATA_TYPE from all_tab_columns tc ");
			sBuffer.append(
			...
			if (resultlist != null && resultlist.size() > 0) {
				...
				if (!resultmap.get("DATA_TYPE").equals(sysZhcxCol.getColsType())) {
					advice = new SysDbmsAdviMessInfo(UUID.randomUUID().toString(), "列配置修改", sysZhcxTab.getTabsDesc(),
							sysZhcxTab.getTabsName(), sysZhcxTab.getJdbcUuid());
					String executeSql = "update sys_dbms_tabs_cols_info t set  t.COLD_TYPE = '"
							+ resultmap.get("DATA_TYPE") + "',t.update_time = sysdate where t.uuid='"
							+ sysZhcxCol.getUuid() + "'";
				...
				if (sysZhcxTab.getTabsRows() != null && sysZhcxTab.getTabsRows() > 10000) {
					...
					if (numNulls != null && numNulls.intValue() != 0) {
						if (numNulls.subtract(new BigDecimal(sysZhcxTab.getTabsRows()).multiply(new BigDecimal(0.6f)))
								.intValue() > 0 && sysZhcxCol.getColsVisible()) {
							advice = new SysDbmsAdviMessInfo(UUID.randomUUID().toString(), "列配置修改",
									sysZhcxTab.getTabsDesc(), tableName, sysZhcxTab.getJdbcUuid());
							StringBuilder sBuilder = new StringBuilder();
							sBuilder.append("-- 表中的空值超过 60% 建议默认列表不展示");
							advice = new SysDbmsAdviMessInfo(UUID.randomUUID().toString(), "列配置修改",
									sysZhcxTab.getTabsDesc(), sysZhcxTab.getTabsName(), sysZhcxTab.getJdbcUuid());
							sBuilder.append(
									"update sys_dbms_tabs_cols_info t set  t.PAGE_LIST = '0',t.update_time = sysdate where t.uuid='"
											+ sysZhcxCol.getUuid() + "'");
							advice.setMessage(sBuilder.toString() + ";");
							advice.setDeleteFlag(0);
							sysAdviceMessDao.save(advice);
				...
				if (sysZhcxCol.getColsDesc() != null && resultmap.get("comments") != null) {
					...
					if (sysZhcxTab.getTabsDesc() == null || "".equals(sysZhcxTab.getTabsDesc())) {
						...
						sBuilder.append(
								"update sys_dbms_tabs_cols_info t set t.COLS_DESC ='" + resultmap.get("comments")
										+ "' ,t.update_time = sysdate where t.uuid='" + sysZhcxCol.getUuid() + "'; \n");
					} else if (resultmap.get("comments") == null || "".equals(resultmap.get("comments"))) {
						sBuilder.append("-- 由于表中字段注释信息没有，建议执行以下语句进行统一：\n");
						sBuilder.append("comment on table " + tableName + "." + sysZhcxCol.getColsName() + "  is '"
								+ sysZhcxCol.getColsDesc() + "';\n");
			...
			startConfixOracleTableCloumnIndexConfig(sysZhcxTab, multiDatasource, sysAdviceMessDao, jdbcTemplate2,
					sysZhcxCol);
	...
	public static void startConfixOracleTableConfig(SysDbmsTabsInfo sysZhcxTab, Map<String, DataSource> multiDatasource,
		...
		StringBuffer sBuffer = new StringBuffer();
		sBuffer.append(
				" select t.owner,t.table_name,t.num_rows,t.blocks*8*1024 as table_space ,tc.comments  from all_tables t ");
		...
		map.put("tablename", tableName);
		SysDbmsAdviMessInfo advice = new SysDbmsAdviMessInfo(UUID.randomUUID().toString(), "表配置修改",
				sysZhcxTab.getTabsDesc(), tableName, sysZhcxTab.getJdbcUuid());
		NamedParameterJdbcTemplate template = new NamedParameterJdbcTemplate(
				multiDatasource.get(sysZhcxTab.getJdbcUuid()));
		...
		if (list != null && list.size() > 0) {
			...
			// 表数据量更新
			if (sysZhcxTab.getTabsRows() == null || sysZhcxTab.getTabsSpace() == null
					|| sysZhcxTab.getTabsRows() != resultmap.get("num_rows")
					|| sysZhcxTab.getTabsSpace() != resultmap.get("table_space")) {
				String executeSql = "update sys_dbms_tabs_info t set  t.tabs_rows = " + resultmap.get("num_rows")
						+ ",t.tabs_space = " + resultmap.get("tabs_space") + ",t.update_time = sysdate where t.uuid='"
						+ sysZhcxTab.getUuid() + "'";
				advice.setExecuteSql(executeSql + ";");
				jdbcTemplate2.execute(executeSql);
				advice.setDeleteFlag(1);
			...
			if (sysZhcxTab.getTabsDesc() != null && resultmap.get("comments") != null) {
				...
				if (sysZhcxTab.getTabsDesc() == null || "".equals(sysZhcxTab.getTabsDesc())) {
					sBuilder.append("-- 由于配置中的信息没有，建议执行以下语句进行统一：\n");
					sBuilder.append("update sys_dbms_tabs_info t set t.tabs_desc ='" + resultmap.get("comments")
							+ "' ,t.update_time = sysdate where t.uuid='" + sysZhcxTab.getUuid() + "'; \n");
				} else if (resultmap.get("comments") == null || "".equals(resultmap.get("comments"))) {
	...
	public static void startConfixMysqlTableConfig(SysDbmsTabsInfo sysZhcxTab, Map<String, DataSource> multiDatasource,
		...
		StringBuffer sBuffer = new StringBuffer();
		sBuffer.append(
				" SELECT t.TABLE_SCHEMA AS OWNER,CONCAT(t.TABLE_SCHEMA,'.',t.TABLE_NAME) AS tabs_name,t.TABLE_ROWS AS  num_rows,t.TABLE_COMMENT AS comments  FROM information_schema.TABLES t ");
		...
		if (list != null && list.size() > 0) {
			...
			if (sysZhcxTab.getTabsRows() == null || sysZhcxTab.getTabsSpace() == null
					...
					|| sysZhcxTab.getTabsSpace() != resultmap.get("table_space")) {
				String executeSql = "update sys_dbms_tabs_info t set  t.tabs_rows = " + resultmap.get("num_rows")
						+ ",t.update_time = current_timestamp()  where t.uuid='" + sysZhcxTab.getUuid() + "'";
				advice.setExecuteSql(executeSql + ";");
				jdbcTemplate2.execute(executeSql);
				advice.setDeleteFlag(1);
	...
	public static void startConfixMysqlTableColumnsConfig(SysDbmsTabsInfo sysZhcxTab,
			...
			JdbcTemplate jdbcTemplate2, List<SysDbmsTabsColsInfo> colList) {
		// 列配置比较建议修正,平台隐藏，实际长度修改(列修改，列配置修改,列统计信息)
		// 列信息处理会有多个同时执行，遮里配置和列的注释都有可能为空，需要对比
		// 统计信息包含陪的实际最大长度 给出建议缩小列的长度类型， 列空值比例，当空超过60% 给出建议平台默认不展示
		// 实际长度暂不处理
		...
		for (SysDbmsTabsColsInfo sysZhcxCol : colList) {
			StringBuffer sBuffer = new StringBuffer();
			sBuffer.append(
					" SELECT tc.TABLE_SCHEMA AS OWNER,tc.TABLE_NAME AS TABLE_NAME,tc.COLUMN_NAME AS COLUMN_NAME,tc.COLUMN_COMMENT AS  comments, tc.COLUMN_TYPE AS DATA_TYPE,tc.COLUMN_KEY FROM information_schema.COLUMNS tc ");
			...
			if (resultlist != null && resultlist.size() > 0) {
				...
				if (!resultmap.get("DATA_TYPE").toString().contains((sysZhcxCol.getColsType().toUpperCase()))) {
					advice = new SysDbmsAdviMessInfo(UUID.randomUUID().toString(), "列配置修改", sysZhcxTab.getTabsDesc(),
							sysZhcxTab.getTabsName(), sysZhcxTab.getJdbcUuid());
					String executeSql = "update sys_dbms_tabs_cols_info t set  t.cols_type = '"
							+ resultmap.get("DATA_TYPE") + "',t.update_time = current_timestamp() where t.uuid='"
							+ sysZhcxCol.getUuid() + "'";
					advice.setExecuteSql(executeSql + ";");
					jdbcTemplate2.execute(executeSql);
					advice.setDeleteFlag(1);
				...
				// 配置列展示
				// if (sysZhcxTab.getTabsRows() != null &&
				// sysZhcxTab.getTabsRows() > 10000) {
				// BigDecimal numNulls = (BigDecimal)
				// resultmap.get("NUM_NULLS");
				// if (numNulls != null && numNulls.intValue() != 0) {
				// if (numNulls.subtract(new
				// BigDecimal(sysZhcxTab.getTabsRows()).multiply(new
				// BigDecimal(0.6f))).intValue() > 0 && 1 ==
				// sysZhcxCol.getPageList().intValue()) {
				// advice = new
				// SysDbmsAdviMessInfo(UUID.randomUUID().toString(), "列配置修改",
				// sysZhcxTab.getTabsDesc(), tableName,
				// sysZhcxTab.getJdbcUuid());
				// StringBuilder sBuilder = new StringBuilder();
				// sBuilder.append("-- 表中的空值超过 60% 建议默认列表不展示");
				// advice = new
				// SysDbmsAdviMessInfo(UUID.randomUUID().toString(), "列配置修改",
				// sysZhcxTab.getTabsDesc(), sysZhcx4Tab.getTabsName(),
				// sysZhcxTab.getJdbcUuid());
				// sBuilder.append("update sys_dbms_tabs_cols_info t set
				// t.PAGE_LIST = '0',t.update_time = sysdate where t.uuid='" +
				// sysZhcxCol.getUuid() + "'");
				// advice.setMessage(sBuilder.toString() + ";");
				// advice.setDeleteFlag(0);
				// sysDbmsAdviMessInfoDao.save(advice);
				...
				if (sysZhcxCol.getColsDesc() != null && resultmap.get("comments") != null) {
					...
					if (sysZhcxTab.getTabsDesc() == null || "".equals(sysZhcxTab.getTabsDesc())) {
						...
						sBuilder.append("-- 由于表中字段注释信息没有，建议执行以下语句进行统一：\n");
						sBuilder.append("alter table " + tableName + "  modify column " + sysZhcxCol.getColsName()
								+ " VARCHAR(500) comment '" + sysZhcxCol.getColsDesc() + "';\n");
					} else if (!sysZhcxCol.getColsDesc().equals(resultmap.get("comments"))) {
						sBuilder.append("-- 由于表中字段注释信息和配置中的信息不一致，建议执行以下语句进行统一：\n");
						sBuilder.append("-- 建议 一 根据表信息 更新配置表中的信息.\n");
						sBuilder.append(
	...
	private static void startConfixMysqlTableCloumnIndexConfig(SysDbmsTabsInfo sysZhcxTab,
		...
		// 索引修改 一般发现索引创建位置处于数据空间下或者处于其他用户下的索引空间 给出建议修改
		// 索引添加主要提醒当配置表中有userIndex列配置上数据时 索引没有及时创建的给出创建的提示信息
		if (resultlist.get(0).get("COLUMN_KEY") != null && !"".equals(resultlist.get(0).get("COLUMN_KEY"))) {
			SysDbmsAdviMessInfo advice = null;
			String tableName = sysZhcxTab.getTabsName();
			StringBuilder sBuilder = new StringBuilder();
			String expactUser = tableName.substring(0, tableName.indexOf("."));
